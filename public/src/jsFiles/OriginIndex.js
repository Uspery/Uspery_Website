...
if(!url.includes("?lang=" + preferredLanguages))
    window.history.pushState('', 'Home', url + "?lang=" + preferredLanguages);
...
function readableRandomStringMaker(length) {
    for (var s=''; s.length < length; s += 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLM-NOPQRSTUVWXYZ0123456789'.charAt(Math.random()*62|0));
    return s;
}